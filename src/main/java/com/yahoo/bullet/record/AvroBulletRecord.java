...
public class AvroBulletRecord implements BulletRecord {
    ...
    /**
     * Constructor that takes in the raw serialized byte[] that represents the data in the AvroBulletRecord. The user is
     * responsible for ensuring that this byte[] was indeed produced by the same {@link #getAsByteArray()} method. The
     * user is also responsible for ensuring that the byte[] was produced by the same version of the AvroBulletRecord as
     * is the version that the user is invoking this constructor on.
     *
     * @param data The serialized contents that represent this AvroBulletRecord.
     */
    public AvroBulletRecord(byte[] data) {
        serializedData = data;
        isDeserialized = false;
    }

    /**
     * Constructor that lets you set an arbitrary {@link Map} of field names to values as the data for the AvroBulletRecord.
     * No checks are performed so this method is <strong>unsafe</strong> to use if you are not absolutely sure the
     * data can be placed into a AvroBulletRecord. This is meant as a convenience method to copy the same data into
     * multiple BulletRecords.
     *
     * @param data The mapping of field names to their typed values that constitutes the data for this AvroBulletRecord.
     */
    public AvroBulletRecord(Map<String, Object> data) {
        this.data = data;
        isDeserialized = true;
    }
    ...
    @Override
    public AvroBulletRecord setStringMap(String field, Map<String, String> object) {
        return set(field, object);
    }
    ...
    private AvroBulletRecord set(String field, Object object) {
